...
public final class DateHelper {
    ...
    public static String getStandardNowTime() {
        return DateHelper.getStandardTimeFormat().format(new java.util.Date());
    }
    ...
    public static ArrayList<Date> getIntervalDay(Date lastExsitDate, Date lastday) throws Exception {
        ...
        int beginDay = DateHelper.getDateDay(lastExsitDate);
        int lastDay = DateHelper.getDateDay(lastday);
        String str = DateHelper.date2String(lastExsitDate, "yyyy-MM");
        ArrayList<Date> al = new ArrayList<Date>();
        for (int i = beginDay; i <= lastDay; i++) {
            al.add(DateHelper.string2Date(str + "-" + i, "yyyy-MM-dd"));
        }
        return al;
    }
}
